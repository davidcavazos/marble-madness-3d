...
#include "terminal/commandobject.hpp"
...
CommandObject::CommandObject(const string& name) :
    m_name(name),
    m_commands(),
    m_attributes()
{
    Terminal::registerObject(m_name, this);
}

CommandObject::~CommandObject() {
    Terminal::unregisterObject(m_name);
}
...
bool CommandObject::runObjectCommand(const size_t idCommand, const std::string& arguments) {
    ...
    cerr << "Object \"" << m_name << "\" has no CommandID " << idCommand << endl;
    return false;
}
