...
public:
    ...
    static bool runCommand(const Command& cmd);
    static bool runCommand(const std::string& cmdString);
...
inline bool Terminal::runCommand(const Command& cmd) {
    return cmd.run();
}

inline bool Terminal::runCommand(const std::string& cmdString) {
    Command cmd(0, 0, "");
    if (cmd.parse(cmdString))
        return cmd.run();
    return false;
}
